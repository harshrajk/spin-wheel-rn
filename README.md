...
| Default | Minimal | Sleek |
| :---: | :---: | :---: |
| ![Default theme](https://raw.githubusercontent.com/harshrajk/spin-wheel-rn/main/assets/theme-default-ios.png) | ![Minimal theme](https://raw.githubusercontent.com/harshrajk/spin-wheel-rn/main/assets/theme-minimal-ios.png) | ![Sleek theme](https://raw.githubusercontent.com/harshrajk/spin-wheel-rn/main/assets/theme-sleek-ios.png) |
| No theme – custom colors | `theme="minimal"` | `theme="sleek"` |
...
| `flickEnabled` | `boolean` | — | Enables velocity-based flick to spin (subset of gesture spin). |
...
| `labelRotation` | `"slice" \| "static"` | — | Controls whether labels rotate with their slice (`"slice"`) or stay upright (`"static"`). |
...
| `hapticFeedback` | `boolean` | — | Triggers a short vibration each time the pointer crosses a segment boundary. |
...
Labels are positioned at ~52% of the wheel radius (mid-slice). Font size scales down automatically so longer labels always fit within their slice boundary, with a 62% chord safe-zone that prevents text from touching segment dividers.
...
- **Label rotation** — use the `labelRotation` prop (`"slice"` | `"static"`) to control whether labels rotate with their slice or stay upright.
